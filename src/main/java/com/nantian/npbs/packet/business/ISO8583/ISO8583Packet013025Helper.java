 ...
 */
@Component
public class ISO8583Packet013025Helper extends ISO8583Packetxxx025Helper {
	...
	protected void unpackField55(Object field, BusinessMessage bm) {
		...
		try {
			...
			bm.setResponseMsg("013025解包55域错误!");
			bm.setResponseCode(GlobalConst.RESPONSECODE_FAILURE);
			logger.error("013025解包55域错误!"+e);
			return;
		}
		bm.setCustomData(sc);
	}
	...
	protected String packField44(BusinessMessage bm) 
		...
		try {
			...
			logger.error("013025打包44域错误：bm.getCustomData()类型转换错误!");
			throw new PacketOperationException("bm.getCustomData()类型转换错误!");
		} catch (Exception e){
			logger.error("013025打包44域错误：bm.getCustomData()强制转换抛出未知异常!");
			throw new PacketOperationException("bm.getCustomData()强制转换抛出未知异常!");
		}
		if(null == ic){
			logger.error("013025打包44域错误：bm.getCustomData()为空值!");
			throw new PacketOperationException("bm.getCustomData()为空值!");
		}
		
		StringBuffer str = new StringBuffer();
		str.append("撤销写卡成功！");//.append(ic.getAccountBalance()).append("\n");
		return str.toString();
	}
	...
	protected String packField55(BusinessMessage bm)
		...
		try {
			...
			logger.error("013025打包55域错误：bm.getCustomData()类型转换错误!");
			throw new PacketOperationException("bm.getCustomData()类型转换错误!");
		} catch (Exception e){
			logger.error("013025打包55域错误：bm.getCustomData()强制转换抛出未知异常!");
			throw new PacketOperationException("bm.getCustomData()强制转换抛出未知异常!");
		}
		if(null == ic){
			logger.error("013025打包55域错误：bm.getCustomData()为空值!");
			throw new PacketOperationException("bm.getCustomData()为空值!");
		}
		...
		try {
			...
			logger.error("013025打包55域错误"+e);
			throw new PacketOperationException("013025打包55域错误!");
		}
