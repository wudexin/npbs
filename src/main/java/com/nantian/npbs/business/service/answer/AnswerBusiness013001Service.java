...
/**
 * 华电IC卡现金查询
 * 保存缴费发卡次数，终端发送末笔请求时返回。
 * @author MDB
 *
 */
@Scope("prototype")
@Component
public class AnswerBusiness013001Service extends AnswerBusiness001Service {

	private static Logger logger = LoggerFactory.getLogger(AnswerBusiness013001Service.class);
	...
	protected void setTempValue(ControlMessage cm, BusinessMessage bm) {
		...
		logger.info("本交易流水[{}] ,数据:[{}],交易日期:[{}]", 
				new Object[]{bm.getPbSeqno(),ct.getTempValue(),ct.getTradeDate()});
		try {
			baseHibernateDao.save(ct);
		} catch (Exception e) {
			logger.info("保存现金缴费临时信息失败！");
			cm.setResultCode(GlobalConst.RESULTCODE_FAILURE);
