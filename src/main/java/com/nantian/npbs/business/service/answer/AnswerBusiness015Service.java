...
@Scope("prototype")
@Component
public class AnswerBusiness015Service extends AnswerBusinessService {

	private static Logger logger = LoggerFactory
			.getLogger(AnswerBusiness015Service.class);
	...
	public void dealBusiness(ControlMessage cm, BusinessMessage bm) {
		// 终端查询交易明细（指定用户号）
		logger.info("终端查询交易明细（指定用户号）！");
		if(!getBusinessInfoByBC(cm,bm)){
			logger.error("终端查询交易明细（指定用户号）失败！");
			return;
		}
		logger.info("终端查询交易明细（指定用户号）成功");
	}
	...
	private boolean getBusinessInfoByBC(ControlMessage cm, BusinessMessage bm) {
		...
		try {
			String queryString = "from TbBiTrade t where t.companyCode = "
					+ shopAccount + " and t.id.tradeDate between " + queryStartDate + " and "
					+ queryEndDate;
			...
			bm.setResponseMsg("终端查询交易明细（指定用户号）出错,请拨打客服电话咨询!");
			cm.setResultCode(GlobalConst.RESULTCODE_FAILURE);
			cm.setResultMsg("终端查询交易明细（指定用户号）出错,请拨打客服电话咨询!");
			logger.error("终端查询交易明细（指定用户号）错,查询表：TB_BI_TRADE,商户号: "
					+ bm.getShopCode());
			return false;
		}
		return true;
