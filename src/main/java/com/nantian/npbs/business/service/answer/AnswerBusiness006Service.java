...
public class AnswerBusiness006Service extends AnswerBusinessService {
	...
	public void dealBusiness(ControlMessage cm, BusinessMessage bm) {
		...
		if (CHANEL_TYPE.WEB.equals(cm.getChanelType())) {
			if (!getWebUserTradeByCustomerNo(cm, bm)) {
	...
	private boolean getWebUserTradeByCustomerNo(ControlMessage cm,
			BusinessMessage bm) {
		String origLocalDate = bm.getOrigLocalDate();// 原交易日期
		String userCode = bm.getUserCode();// 用户号
		String packageNum = bm.getPackageNum();// 第packageNum次发包，packageNum为3位001为第一次
		...
		System.out.println(num);
		try {
			String numsql = "select * from tb_bi_trade t where  "
					+ " t.trade_Date = '"
					+ origLocalDate
					+ "' and t.company_code = '"
					+ bm.getShopCode()
			...
			String queryString = "select * from (select a.*,rownum rn from (select * from tb_bi_trade t where  "
					+ " t.trade_Date = '"
					+ origLocalDate
					+ "' and t.company_code = '"
					+ bm.getShopCode()
